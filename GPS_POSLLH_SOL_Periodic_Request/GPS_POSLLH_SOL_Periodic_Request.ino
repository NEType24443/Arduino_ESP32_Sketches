...
const byte posllh_start[] = {0xB5, 0x62, 0x06, 0x01, 0x03, 0x00, 0x01, 0x02, 0x01, 0x0E, 0x47}; // B5 62 06 01 03 00 01 02 01 0E 47 -> POSLLH START
const byte posllh_stop[]  = {0xB5, 0x62, 0x06, 0x01, 0x03, 0x00, 0x01, 0x02, 0x00, 0x0D, 0x46}; // B5 62 06 01 03 00 01 02 00 0D 46 -> POSLLH STOP
const byte    sol_start[] = {0xB5, 0x62, 0x06, 0x01, 0x03, 0x00, 0x01, 0x06, 0x01, 0x12, 0x4F}; // B5 62 06 01 03 00 01 06 01 12 4F -> SOL    START
const byte    sol_stop[]  = {0xB5, 0x62, 0x06, 0x01, 0x03, 0x00, 0x01, 0x06, 0x00, 0x11, 0x4E}; // B5 62 06 01 03 00 01 06 00 11 4E -> SOL    STOP
...
byte calcChecksum(unsigned char* CK, uint8_t data_struct) {
  int payloadSize = 0;
  ...
  while ( GpsUart.available()){
    ...
    // Sync with header after success.
      ...
      if ( fpos == (payloadSize + 2) ) {
        calcChecksum(checksum, data_struct);
      }
      ...
      // Carriage is after the second checksum byte, which has been read, check if it equals to ours.
        ...
        if ( c == checksum[1] ) {
          ...
          //checksum[1] = 0;
          match_counter++;
          Serial<<endl<<F("C --> Checksum match !")<<endl<<F("Match Counter")<<_WIDTH(match_counter,5)<<F("\tNot Match Counter")<<_WIDTH(not_match_counter,5)<<endl;
          return true;
        }
      }
      // Reset the carriage if it is out of a packet.
      else if ( fpos > (payloadSize + 4) ) {
        Serial<< F("E --> Carrige has been reset") <<endl;
        fpos = 0;
      }
    }
  }
  not_match_counter++;
  Serial<<endl<<F("No checksum match")<<endl<<F("Match Counter")<<_WIDTH(match_counter,5)<<F("\tNot Match Counter")<<_WIDTH(not_match_counter,5)<<endl;
  return false;
}
